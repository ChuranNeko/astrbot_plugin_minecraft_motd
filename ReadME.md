...
* **自主 HTML 渲染**：使用 Playwright 浏览器引擎本地渲染，完全不依赖外部服务
* **异步并发**：高性能的并发探测，提升响应速度
* **现代化界面**：基于 HTML/CSS 的精美卡片设计，支持动画效果和渐变配色
* **高质量输出**：浏览器级渲染质量，支持服务器图标和自定义字体
...
> **v2.0.0 重大更新**：
> - ✨ 使用 **Playwright + Jinja2** 自主实现 HTML 渲染
> - 🚀 完全本地化，不依赖任何外部渲染服务
> - 🎨 支持完整的 HTML/CSS 特性，界面更加精美
> - ⚡ 渲染质量和性能大幅提升
> - 🔧 移除了 `Pillow` 和 `requests` 依赖

### 首次安装额外步骤

安装插件后，需要额外安装 Playwright 浏览器引擎（仅需一次）：
...
* **Playwright 渲染引擎**：使用 Chromium 无头浏览器渲染，生成高质量图片
* **完全自主实现**：不依赖任何外部 API 或云服务，完全本地化渲染
* **现代化设计**：采用渐变配色、圆角卡片、阴影效果等现代 UI 元素
* **动画效果**：支持 CSS 动画，在线状态指示器呈现呼吸灯效果
* **原生字体**：内置 Minecraft 官方字体，通过 CSS @font-face 加载
* **自动图标**：智能获取服务器 favicon，失败时显示默认 emoji 图标
* **响应式布局**：基于 Flexbox/Grid 的响应式布局，完美展示各类信息
* **完整 CSS 支持**：支持所有现代 CSS 特性，包括渐变、阴影、动画等
* **颜色码清理**：自动去除 Minecraft 颜色码，保持文本清洁
* **高度可定制**：可通过修改 HTML/CSS 模板实现任意样式
...
1. **数据采集**：通过 mcstatus 库探测服务器状态
2. **模板渲染**：使用 Jinja2 将数据注入 HTML 模板
3. **浏览器渲染**：Playwright 启动 Chromium 无头浏览器
4. **截图生成**：浏览器将 HTML 渲染为高质量 JPEG 图片
5. **消息发送**：通过 AstrBot API 发送图片到聊天平台

### 技术栈

| 组件 | 技术 | 用途 |
|------|------|------|
| 服务器探测 | mcstatus | 探测 MC 服务器状态 |
| DNS 解析 | dnspython | SRV 记录查询 |
| 模板引擎 | Jinja2 | HTML 模板渲染 |
| 浏览器引擎 | Playwright (Chromium) | HTML 转图片 |
| 异步处理 | asyncio | 高性能并发 |

### 优势对比

| 特性 | v1.x (PIL) | v2.0 (Playwright) |
|------|-----------|-------------------|
| 渲染质量 | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| UI 灵活性 | ⭐⭐ | ⭐⭐⭐⭐⭐ |
| CSS 支持 | ❌ | ✅ 完整支持 |
| 动画支持 | ❌ | ✅ CSS 动画 |
| 自定义难度 | 困难 | 简单（HTML/CSS） |
| 依赖外部服务 | ❌ | ❌ |
| 字体渲染 | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
